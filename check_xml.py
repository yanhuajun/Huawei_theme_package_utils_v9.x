...
framework_res_hwext_change_v91_v10=[
	...
	{
		'orgin':'',
		'replace':[{'tag':'<color name="emui_color_bg">{color}</color>','color':'{color}'}],
	...
	{
		'orgin':'',
		'replace':[{'tag':'<color name="emui_accent">{color}</color>','color':'{color}'}],
		'target':'framework-res-hwext',
		'color':'<color name="hwtoolbar_background">(.*)</color>'
	},
...
framework_res_hwext_change_forSystemUi_v91_v10=[
	# 默认颜色则直接修改  非默认颜色保持不变
	# 颜色必须保持一致开始
	{
		'orgin':'<color name="emui_appbar_bg">#F2F2F2</color>',
	...
	# 主题色 开关已开的文字颜色 ，时间的文字颜色  暂不处理
	# {
	# 	'orgin':'',
	# 	'replace':[{'tag':'<color name="emui_text_primary">{color}</color>','color':'{color}'}],
	# 	'target':'',
	# 	'color':'<color name="mms_text_counter_color">(.*)</color>'
	# },
	{
		'orgin':'',
		'replace':[{'tag':'<color name="emui_color_bg">{color}</color>','color':'{color}'}],
...
framework_res_hwext_change_forMms_v91_v10=[
	# 默认颜色则直接修改  非默认颜色保持不变
	# 颜色必须保持一致开始
	{
		'orgin':'<color name="emui_appbar_bg">#F2F2F2</color>',
	...
	{
		'orgin':'',
		'replace':[{'tag':'<color name="emui_color_fg">{color}</color>','color':'{color}'}],
		'target':'framework-res-hwext',
		'color':'<color name="emui_color_gray_1">(.*)</color>'
	},
...
contacts_normalConfig_v91_v10 = [
	...
	{
		'orgin':'',
		'replace':[{'tag':'<color name="hwfab_shadow_start">{color}</color>' , 'color':'000000FF'}],
		'target':'framework-res-hwext',
		'color':'<color name="primary_emui_light">(.*)</color>'
	},
	{
		'orgin':'',
		'replace':[{'tag':'<color name="hwfab_shadow_end">{color}</color>' , 'color':'000000D0'}],
		'target':'framework-res-hwext',
		'color':'<color name="primary_emui_light">(.*)</color>'
	},
	...
	{
		'orgin':'',
		'replace':[{'tag':'<color name="dial_hwfab_shadow_start">{color}</color>' , 'color':'000000FF'}],
		'target':'framework-res-hwext',
		'color':'<color name="primary_emui_light">(.*)</color>'
	},
	{
		'orgin':'',
		'replace':[{'tag':'<color name="dial_hwfab_shadow_end">{color}</color>' , 'color':'000000D0'}],
		'target':'framework-res-hwext',
		'color':'<color name="primary_emui_light">(.*)</color>'
	},
...
hwvoipservice_framework_res_hwext_v91_v10 = [
	{
		'orgin':'',
		'replace':[{'tag':'<color name="emui_appbar_bg">{color}</color>' , 'color':'{color}'}],
		'target':'com.android.contacts/framework-res-hwext',
		'color':'<color name="hwtoolbar_background">(.*)</color>'
	},
	{
		'orgin':'',
...
mms_normalConfig_v91_v10 = [
	...
	{
		'orgin':'',
		'replace':[{'tag':'<color name="mms_color_primary_dark">{color}</color>' , 'color':'{color}'}],
		'target':'',
		'color':'<color name="fab_new_message_center_color">(.*)</color>'
	},
...
systemui_normalConfig_v91_v10 = [
	...
	{
		'orgin':'',
		'replace':[{'tag':'<color name="qs_tile_tint_on">{color}</color>' , 'color':'{color}'}],
		'target':'framework-res-hwext',
		'color':'<color name="emui_color_bg">(.*)</color>'
	},
	...
	{
		'orgin':'',
		'replace':[{'tag':'<color name="volume_image_color">{color}</color>' , 'color':'{color}'}],
		'target':'',
		'color':'<color name="notification_header_icon_color">(.*)</color>'
	},
	{
		'orgin':'',
...
normalConfig_dic = {
	...
	'telecom_normalConfig_v91_v10':telecom_normalConfig_v91_v10
}
...
def doTransform( filePath  , configParam ):
	...
	for item in configParam:
		...
		if item['color'] != None and item['color'] != '' :
			...
			if findTargetColor =='null' or findTargetColor=="":
				print 'can not find the target color ,pass this item...'
				continue;
		...
		if item['orgin'] == '':
			...
			for param in item['replace']:
				...
				if configFileContent.find(replaceStr) > 0:
					...
					configFileContent+="\n\t<!-- hwv91-v10 autoadd ,获取更多工具帮助或技术支持： qq:756762990@qq.com  name:cla  --> \n\t" + replaceStr + "\n"
			...
			for param in item['replace']:
				...
				if configFileContent.find(replaceStr) > 0:
					...
					print 'not exists  , add...'
					configFileContent+="\n\t<!-- hwv91-v10 autoreplace  ,获取更多工具帮助或技术支持： qq:756762990@qq.com  name:cla --> \n\t" +replaceStr + "\n"
...
def checkdot9pic():
	for item in dot9picList:
		inputFilePath = os.path.join(toolPath,item['input'])
...
def updateLauncher():
	...
	tempLauncherPath = os.path.join(toolPath , 'launcher' )
	command = ''
	if not isWindows :
...
def updateHwvoipservice():
	...
	# 	mkdir(targetPath)
	if isWindows:
		command = 'xcopy \'' + toolPath+ '\\com.huawei.hwvoipservice\\*.*\' \''+ currentProjectFilePath + '\\\' /Y'
	else:
		command = 'cp -R \'' + toolPath+ '/com.huawei.hwvoipservice\' \''+ currentProjectFilePath + '/\''
	print 'command:' + command
	os.system(command)
	...
	os.system(command)
	if isWindows:
		command = 'xcopy \'' + currentProjectFilePath + '\\com.android.contacts\\framework-res-hwext\\res\\drawable-xxhdpi\\background_emui.9.png\' \''+ currentProjectFilePath + '\\com.huawei.hwvoipservice\\framework-res-hwext\\res\\drawable-xxhdpi\\\' /Y'
	else:
		command = 'cp \'' + currentProjectFilePath + '/com.android.contacts/framework-res-hwext/res/drawable-xxhdpi/background_emui.9.png\' \''+ currentProjectFilePath + '/com.huawei.hwvoipservice/framework-res-hwext/res/drawable-xxhdpi/\' '
	print 'command:' + command
	os.system(command)
	...
	print 'load configFilePath:' + configFilePath
	configContent = open(configFilePath).read()
	print 'configContent:' + configContent
	...
	btnOpenBgColor = getOrginColorFromConfig(os.path.join(currentProjectFilePath,'com.android.mms' , 'framework-res-hwext' , 'theme.xml') ,'','<color name="emui_accent">(.*)</color>','#272822')
	btnCloseBgColor = getOrginColorFromConfig(os.path.join(currentProjectFilePath,'com.android.mms' , '' , 'theme.xml') ,'','<color name="mms_color_primary_dark">(.*)</color>','#272822')
	btnCloseBgColor += "D9"

	if color =='null' or color == '':
		return null;
	configContent = configContent.replace('{color}' , color ,1000)
	configContent = configContent.replace('{lightcolor}' , btnOpenBgColor ,1000)
	configContent = configContent.replace('{front_color}' , getWhiteOrBlack(color) ,1000)

	

	# configContent = configContent.replace('{lightcolor}' , "#004000" ,1000)
	# configContent = configContent.replace('{front_color}' , "#a92608" ,1000)
...
def getWhiteOrBlack(orgincolor):
	print 'getWhiteOrBlack:\n'
	print 'orginColor:hex:' + orgincolor + ',rgb:' + Hex_to_RGB(orgincolor)
	rgbArray = Hex_to_RGB(orgincolor).split(',')
	r = int(rgbArray[0])
	g = int(rgbArray[1])
...
if len(sys.argv) > 1:
	...
	toolPath = os.getcwd()
print 'currentProjectFilePath :' + currentProjectFilePath +",toolPath:" + toolPath
print '\n'
# 判断是否windows
...
for item in frameworkResHwextConfigFilePathList:
	print 'replace iten:' + item 
	if item.find('com.android.systemui')>=0:
		print 'is a systemui config'
		doTransform(item , framework_res_hwext_change_forSystemUi_v91_v10)
	elif item.find('com.android.mms')>=0 :
		print 'is a mms config'
		doTransform(item , framework_res_hwext_change_forMms_v91_v10)
	else:
		print 'not a normal config'
